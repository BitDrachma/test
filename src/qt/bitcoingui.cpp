...
void BitcoinGUI::createActions()
{
    ...
    aboutAction->setStatusTip(tr("Show information about bitgold"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for bitgold"));
...
void BitcoinGUI::blocksIconClicked()
{
   ...
   message(tr("Extended Block Chain Information"),
       tr("Client Version: %1\n"
          ...
          .arg(tr("5.0000000")) // Hard Coded as BGL is always 5, but should use GetProofOfWorkReward
    ...
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            walletStack->gotoSendCoinsPage();
             ...
             message(tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid bitgold address or malformed URI parameters."),

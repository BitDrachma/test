...
unsigned int nStakeMinAge = 60 * 60 * 24 * 1; // minimum age for coin age - 10 days
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, bool bCoinYearOnly)
{
    ...
    if(fTestNet || nTime > PROTOCOL_SWITCH_TIME)
    {
        ...
        // bitgold: reward for coin-year is cut in half every 64x multiply of PoS difficulty
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "bitgold", CClientUIInterface::MSG_WARNING);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
	   ...
	   if (true  && (block.GetHash() != hashGenesisBlock)) {
        ...
        {
           assert(block.hashMerkleRoot == uint256("0x4cbe5de997ef680741e3dc7add1754f5315d30be64b8a16af0c8a336273d51a5"));
        }
